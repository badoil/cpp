...
void storeByLRef(string & s)     // 이때 레퍼런스로 파라미터 넘기기에 복사는 안일어남, 하지만 함수 내부에서 힙의 "abc"복사되고 b가 이를 가리킴
{
  string b = s;
}

void storeByRRef(string && s)    // 그런데 함수가 콜 되면서 스택프레임이 올라가고 거기에 알밸류 레퍼런스(&& s)가 들어있음
{                                  // && s 가 힙의 "abc"를 가리키게 됨
...
int main()
{
  ...
  storeByLRef(a);     // 이때 레퍼런스로 파라미터 넘기기에 복사는 안일어남, 하지만 함수 내부에서 힙의 "abc"복사되고 b가 이를 가리킴
  
  storeByRRef("abc"); // std::move(a) 알밸류만 들어가야하기에
