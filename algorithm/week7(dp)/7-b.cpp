...
int main () {
    ...
    b[1][2][0] = 1;             // 파이프의 끝 지점 위치 (1, 2)위치에 있고 방향은 가로(0)
    for(int i=1; i<=n; i++) {
        for(int j=1; j<=n; j++) {
            // 가로방향, 가로/대각으로 이동 가능
    ...
    ret = b[n][n][0];           // (n, n)은 세가지 방향에서 도달할 수 있음
    ret += b[n][n][1];
    ret += b[n][n][2];
...
// 파이프가 두칸을 차지하지만, 파이프의 끝지점만 신경쓰면 됨
...
// 참고
// https://nanyoungkim.tistory.com/183 