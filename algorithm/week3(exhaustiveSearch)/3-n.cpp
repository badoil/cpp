...
void go(int s, int e, int level) {
    if (s>e) return;                    // 시작s이 끝e 보다 작아야 하므로, 시작이 커지면 리턴 
    if (s==e) {                         // 하나만 남으면 그 값을 해당 레벨에 푸시
...
// 이 문제는 inorder 문제, 주어진 숫자 열을 inorder 순서로 트리를 만드는 문제
// 노드의 갯수가 항상 홀수
// 각 레벨에 노드 숫자들의 mid 값을 넣어주고
// 맨끝에 리프 노드들도 해당 레벨에 푸시
