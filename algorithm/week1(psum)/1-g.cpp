...
int main () {
    ...
    cin >> pat;
    int star = pat.find('*');
    pre = pat.substr(0, star);
    suf = pat.substr(star+1);
    
    for (int i=0; i<n; i++) {
        cin >> s;
        if (pre.size()+suf.size() > s.size()) cout << "no\n";   // 반례
        else {
            if (pre==s.substr(0, pre.size()) && suf==s.substr(s.size()-suf.size())) cout << "yes\n";
            else cout << "no\n";
        } 
    }
    return 0;
...
// 여기서 반례를 처리해주는 부분이 필요, a*a 패턴일때 문자열a도 패턴과 일치한다고 잘 못 판단하는데 이를 걸러줌
// 패턴의 앞과 뒤가 일치하는 문자열을 찾음