// 문제
// One of the most important data structures in computer science is the tree.
// You already dealt with binary trees in the qualification round.
...
// 입력
// The first line will contain an integer T representing the number of graphs to check.
// There will be at most 10 graphs in each test case.
// Each of the graph will be represented as follows:

// The first line will contain an integer N with the number of nodes in the graph.
// The number of nodes will be between 1 and 1,000.
...
int main()
{
    ...
    if (m = n - 1 && cnt == 1) // 엣지와 버텍스 갯수 체크 & dfs 함수 호출한 cnt 1이면 한번에 모든 노드 방문한것
        puts("tree");
    else
        puts("graph");
}
...
// 즉 dfs 한번에 모든 노드 방문 가능
// 그리고 Edge = Vertex - 1 성립함
// dfs 걸어서 확인해보면 됨
