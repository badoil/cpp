...
int t, n, m, c, s, g, h, a, b, d, dist[2004]; 
...
int main() {
    ...
    while(t--) {
        ...
        for (int i=0; i<m; i++) {
            cin >> a >> b >> d;
            d *= 2;                 // 두배 만들어준 후에
            if (a==g && b==h) d--;  // 주어진 교차로 사이의 도로 거리를 홀수로 만듬
            else if (a==h && b==g) d--; // 양방향이므로
        ...
        for (int i=0; i<c; i++) {
            if (dist[candi[i]] % 2 == 1) cout << candi[i] << "\n";      // dist[next] = dist[here] + dist_there 이므로 dist_there가 홀수면 전체값도 홀수고, 주어진 도로를 지난거임
        }
...
// 이 문제는 주어진 도로를 통과해서
// 출발지에서 후보지들중에 최단 경로로 가는 
// 도착지를 구하는 문제

// 주어진 도로를 통과했는지 여부를 거리 d를 홀수로 만들어서 체크
// dist[next] = dist[here] + dist_there 이므로 dist_there가 홀수면 전체값도 홀수고, 주어진 도로를 지난거임
