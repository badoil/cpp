...
int t, n, a[130][130], dist[130][130], t;
...
int main () {
    while (++t) {
        ...
        pq.push(make_pair(a[0][0], 0));     // 출발지의 비용
        dist[0][0] = a[0][0];
        while(pq.size()) {
            int hereY = pq.top().second / 1000;     // 1차원 위치정보를 2차원으로 변환
            int hereX = pq.top().second % 1000;     // 1차원 위치정보를 2차원으로 변환
            int dist_here = pq.top().first;
            if (dist[hereY][hereX] != dist_here) continue;
            pq.pop();
            for (int i=0; i<4; i++) {
                int ny = hereY + dy[i];
                int nx = hereX + dx[i];
                if (ny<0 || nx<0 || ny>=n || nx>=n) continue;
                int dist_there = a[ny][nx];
                if (dist[ny][nx] > dist[hereY][hereX] + dist_there) {
                    dist[ny][nx] = dist[hereY][hereX] + dist_there;
                    pq.push(make_pair(dist[ny][nx], ny*1000+nx));   // 2차원 위치를 1차원으로 바꿈
                }
            }
        }
        printf("problem %d: %d", t, dist[n-1][n-1]);       // 요구하는 출력 조건에 맞게 출력
    }
    return 0;
}


// 이 문제는 가중치가 다른 최단 경로 구하는 문제
// 즉 다익스트라
// 다만 이전과 다르게 인접리스트가 아니라
// 2차원 배열 형태로 주어짐
// 이를 1차원으로 바꾸고 나중에 다시 2차원으로 변환
// 나머지는 다익스트라와 동일