...
int main() {
    ...
    for(int i=0; i<101; i++) {                              // 테스트 케이스가 비의 높이 100까지 임, 여기서 i=0 즉 비가 안오는 경우도 생각하는 것이 포인트
...
// 이 문제는 비가 안오는 경우라는 반례를 생각해야함
// 문제에서 비가 반드시 온다는 말이 없기때문임

// 테스트 케이스가 비의 높이 100까지 임, 여기서 i=0 즉 비가 안오는 경우도 생각하는 것이 포인트
// 이때 ret=0 으로 초기화되어 있기 때문에
// i=0 일때 cnt가 1이 되고 이것이 ret 값에 더해짐
// 이 케이스를 생각하지 않으면 오답