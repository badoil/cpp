...
void dfs(int y, int x) {
    ...
    if(a[y][x]==1) {
        v.push_back({y,x});
        return;                     // 가장 겉에 있는 치즈 발견하면 멈춤, 이게 핵심
    }
    for (int i=0; i<4; i++) {
        ny = y+dy[i];
        nx = x+dx[i];
...
int main() {
    ...
    while(true) {
        ...
        for(auto c: v) {
            ...
            cnt2++;                              // 그 치즈조각 갯수 카운팅, 얘는 매번 새롭게 갱신되므로 치즈가 다 녹는 마지막 시간 바로 이전에 cnt2가 정답
...
// dfs 로 가장 겉에 있는 치즈만 발견하는것이 핵심
// 그리고 그것들을 v벡터에 넣어주고 
// 그것을 반복하면서 cnt와 cnt2를 세주는 거임